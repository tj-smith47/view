...
| 24 | Allowlist the environment at the bench/oracle spawn funnel | Landed in `efb594d`, hardened across the review-fix rounds through `9e389eb` and after (`HOME` re-pointed at `env::hermetic_home` in `0299417`, both funnels pinned to run the preparation refusals). `task ci` green on dev-linux (731 tests at `9e389eb`) and natively on windows-msvc (664 tests at `9e389eb`). Closed |
