...
- [ ] **GitHub Actions CI workflow verification**: unverified on real GitHub
  ...
  First-push flow for the bench job (both legs run `--gate --bootstrap`):
  ...
  and remove `--bootstrap` plus the "Upload recorded baseline" step from ci.yml
  in that same commit. If the null-pair calibration refuses on a shared runner
