...
| Keypress → cell change, bench fixtures (`echo.minimal`, `echo.heavy`) | **Diagnostic:** the row above read on the two bench legs, at the same bound — p99 ≤ 8 ms; ratio_p50 ≤ 1.10× the paired bare-nvim run. They hold the plugin set constant, which is what makes the felt row's own number attributable and what the `[[shortfall]]` ledger in `crates/view-bench/budgets.toml` is written against; the moment itself is the row above, measured under a config a person runs, and these legs are never quoted as the claim. | pty harness, paired against bare nvim in the same run, samples interleaved |
...
| Embedded engine startup cost (`server_delta_ms`) | **≤ 5 ms on a controlled class.** Diagnostic only: `NVIM STARTED` under view minus `NVIM STARTED` under nvim's own TUI, from `--startuptime` on both sides of the same interleaved run under the same config. It decomposes the settled-screen row above and carries no claim of its own — the settled screen can never be faster than the server that draws it, so a regression inside the engine's startup (a surface externalized before the config is sourced, a hook that makes a plugin do more work) reads here first. Signed: an embedding that costs the engine nothing sits at or below zero. | pty harness, paired |
...
| view-side memory (PSS), 10 buffers, post-workload | ≤ 150 MB **Resource:** a footprint, never a speed -- this row is never stated as faster than anything, and view embeds an engine it can never be smaller than. | bench suite |
