...
| Remote editing: view's own local footprint once the engine runs remotely | ≤ 6 MB PSS, dev-linux — headroom over the recorded 4.962 MB `memory.minimal` local-spawn figure this class inherits from: moving the engine off-host must not grow view's own process footprint, since view holds no more buffer state when the engine is remote than when it is local (resource-claim ladder rung 4). Measured by the `remote_memory` row against the same 10-buffer workload `memory.minimal` uses, `--remote`-armed over the committed stub-ssh double in CI (`view_oracle::remote`) with an opt-in real-SSH leg for local/acceptance runs (`VIEW_REMOTE_TEST_HOST`) | bench suite (`remote_memory` row, pty) |
