...
| 24 | Allowlist the environment at the bench/oracle spawn funnel | Landed in `efb594d`, review findings closed in `d32daf0`: the funnel sweeps every host variable the allowlist does not name on both spawn builders, and neutralises the git configuration layers so a child's own subprocesses read nothing through `HOME`. Verified by `task ci` on dev-linux and natively on windows-msvc. Closed |
