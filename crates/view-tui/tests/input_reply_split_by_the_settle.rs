...
use view_tui::tiers::{Probe, ReplySource, PROBE_DEADLINE};
...
/// The probe's reader for this test: the pty slave, on descriptor 0.
///
/// Polls before it reads because the slave is in raw mode with `VMIN=1`, so
/// a read with nothing queued would block past the probe's own window.
struct SlaveSource;

impl ReplySource for SlaveSource {
...
fn the_head_of_an_answer_still_arriving_at_the_settle_crosses_the_handover() {
    ...
    let probe = Probe::start(SlaveSource, &mut queries, PROBE_DEADLINE, None).unwrap();
    ...
    let mut input = InputSource::open_listening(outcome.caps, outcome.partial_reply).unwrap();
