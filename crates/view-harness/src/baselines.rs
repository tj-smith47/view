...
mod tests {
    ...
    /// band's own 2x half-width, and a survives-its-own-ratchet check in
    /// prose, so a factor edited away from what its draws support surfaced
    /// one CI bench leg later, as a breach or as a regression admitted in
    /// silence.
