...
/// so on Windows an entry and a running process are the same answer.
///
/// Deliberately not `kill -0`: that is unix-only, and gating a reaping pin
/// on it takes the proof away from exactly the platform whose process
/// lifetime rules differ most. Here rather than in each test crate, because
/// the three reaping pins that ask this question were three copies of it,
/// two of them Linux-only -- which is what kept those two pins off the
/// platforms this tie was built for.
#[must_use]
