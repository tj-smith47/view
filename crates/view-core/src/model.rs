...
pub struct Model {
    ...
    /// every surface whose feature ships on, and the multigrid option with
    /// them -- a default naming any other set would answer for a session
    /// view does not ship.
...
impl Model {
    ...
    /// Records the `ext_*` set this session holds.
    ///
    /// Written with exactly the list the engine was given, twice over: the
    /// binary calls it once the `[native]` table has been read and before
    /// `nvim_ui_attach` is issued, and
    /// [`update::look`](crate::update) calls it again whenever a look flip
    /// takes a surface or hands one back through
    /// [`RpcCall::SetUiExt`](crate::msg::RpcCall::SetUiExt). A session that
    /// recorded a different set from the one it sent would answer
...
pub struct EngineModel {
    ...
    /// When the key view forwarded went out, while it is still waiting for
    /// the redraw batch that answers it, and `None` once one has arrived.
    ///
...
mod tests {
    ...
    /// view actually ships -- every surface whose feature ships on, and the
    /// multigrid option with them -- which is what a `Model` built by a
    /// test, a bench or an oracle run is standing in for. A surface held
    /// back is nvim's, and must answer `owns` as nvim's.
    #[test]
    fn an_unattached_model_owns_the_shipped_attach_set() {
        let model = Model::new();
        let shipped = crate::native::ext::shipped_multigrid();
        for &surface in crate::native::ext::ALL_MULTIGRID {
            assert_eq!(
                model.owns(surface),
                shipped.contains(&surface),
                "{surface:?} must default to owned exactly when it ships attached"
            );
        }
        assert_eq!(
