...
reasoning keeps an entry of its own:
...
A prompt appears the moment you send it. The header's first row keeps the
session's accounting as the agent reports it:
...
The composer never takes more than half the rows the panel has left after its
...
Scrolling up holds the window where you left it, so an agent that keeps
talking leaves the text you are reading in place, and the last row says
so:
...
keys while it is up, so none of the four scroll then. An open review leaves
them alone: it is read and decided in the file itself, so the panel beside
it scrolls the way it always does.
...
of the single-letter space, buffer-locally, for as long as the review is
open:
...
A file being deleted counts as a change like any other: it is noticed on the
same terms, and the notice below is what answers it. A tool that *replaces* a
...
the second look above confirms it, so you have it before your next `:w`.
