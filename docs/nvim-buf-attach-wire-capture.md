...
which attributed `more: true` to `:%s`-style batching without having captured it
...
undemonstrated by this document; treat it as an open question, not an
...
individually-applied edits nvim is replaying one line at a time, not one logical
change nvim merely reports in pieces. Reading the buffer back afterward confirms
the full revert (`["row0", "row1", "row2", ...]`, 200 lines).
