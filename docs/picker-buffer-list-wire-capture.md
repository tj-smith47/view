...
`EngineHandle::request_buffer_list` issues to resolve the picker's
`Source::Buffers` corpus.
...
`name`. `decode_buffer_list_reply` must not choke on an empty string, and the
picker's `PickerItem` label for it needs a placeholder (`[No Name]`, matching
nvim's own convention); an empty row would leave the entry unreadable.
...
`:help`), but only two come back: the help buffer's `'buftype'` is `"help"` and
nvim does not mark it `buflisted`. **This confirms the `buflisted` filter in
the Lua chunk is load-bearing**, and does real work; a picker built without it
would surface `:help`/quickfix/terminal scratch buffers a user never asked to
jump to, exactly what `:ls` (and every Telescope-style buffer picker) also
excludes by default. The order returned is buffer-number order
(`nvim_list_bufs`'s own order), distinct from MRU; the picker does not attempt
recency ranking; nucleo's match score is the only ordering signal.
...
handling). Since the Lua chunk here is fixed, constant source (no interpolated
caller data, same discipline as `REGISTER_MAPPINGS_CHUNK`), a non-nil error on
this call can only mean something is wrong with the engine connection itself;
the request's arguments carry no blame. `decode_buffer_list_reply` degrades an
error reply to an empty list, the same "confirmed nothing" default
`decode_mapping_claims` uses, so a picker session gets an empty list with an
explanation and stays out of the stuck-with-neither state.
...
- The reply's `name` field is used as-is for a real path; an empty string is
  ...
  list (an unsaved scratch buffer is still a legitimate jump target).
- `buflisted` filtering happens in the Lua chunk itself (cheaper than shipping
  every buffer over the wire and filtering client-side), matching `:ls`'s own
  default buffer set.
