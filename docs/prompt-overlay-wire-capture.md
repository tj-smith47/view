...
run of the pinned binary. Source of truth for the modal prompt overlay
(`OverlayKind::Prompt`) implementation.
...
spawns `nvim --embed --clean` with the same hermetic `XDG_*`/`HOME` isolation
`EngineConfig::isolated()` uses, attaches as a real UI with
`ext_linegrid`/`ext_cmdline`/`ext_popupmenu`/`ext_messages`/`ext_tabline`, and
drives `nvim_input` as a **notification**; fire-and-forget, matching production
`RpcCall::Input` exactly, since a *request* (`nvim_command`, `nvim_eval`) would
block on its own reply while nvim is itself blocked inside a prompt's input
loop and never send one back. Output below is the flattened `redraw` batches,
in wire order, split into named sections.

## 1. `:confirm()`; the baseline choice prompt

Driven: `:call confirm("Save changes?", "&Yes\n&No")<CR>`, then an unmatched
key (`z`), then the accepted answer (`y`).
...
**Confirms the as-built `MessageEntry::is_prompt` doc exactly:** the question
arrives once as `msg_show` kind `"confirm"`; the answer line is a separate,
repeated `cmdline_show`; an unmatched key re-arms via `cmdline_hide` +
`cmdline_show` alone, with **no second `msg_show` **; the session stays alive
and responsive after resolution.
...
Both resolve the dialog; neither re-arms it. The session stays alive.
`cmdline_hide`'s second argument is nvim's own `abort` flag: `True` for
`<Esc>`, `False` for every other case captured (a matched answer, the bracketed
default via `<CR>`, and a re-arm). `accepts()` therefore treats `<CR>` and
`<Esc>` as resolving keys on every choice prompt, beyond just the literal
accelerator letters.
...
Driven: `:call inputlist(["Select an option:", "1. one", "2. two"])<CR>`, then
`1<CR>`.
...
Same `msg_show` kind (`"confirm"`) as `:confirm()`; the difference is entirely
in the paired `cmdline_show` prompt text. Two structural facts distinguish this
class and drive `PromptState`'s parser:

- The prompt text does not follow the `[X]abel, (Y)abel: ` bracket/paren
  convention at all (its first character is `T`, distinct from `[` or `(`), so
  a parser built against capture 1's shape correctly refuses to classify it as
  a choice list.
- Nvim itself echoes what's typed back through `cmdline_show`'s `content` on
  every keystroke (`content: [[0, '1', 0]]` above); the authoritative source
  `PromptState` reads for its free-text `input`; independently tracking typed
  characters would risk drift from what nvim's own cmdline buffer holds.
...
With `ext_messages` attached (which view uses, and `cmdheight=0` forces), nvim
trusts the UI to hold and scroll its own message history, with no blocking for
an on-grid "-- More --"/hit-enter acknowledgement, so there is no
return-prompt-class blocking dialog left to route on this engine at all.
`MessageEntry::is_prompt`'s existing `kind == "confirm"` check was already
correct as-built; it never referenced `return_prompt`; so this finding requires
no code change, only this record that the interface comment's "confirm /
return_prompt / inputlist-class" phrasing describes one reachable wire kind
(`"confirm"`) covering exactly two behaviorally distinct prompt shapes: choice
and free-text.

## Conclusions for the implementation

- `PromptState::from_entry` gates on `MessageEntry::is_prompt()` alone
  (`kind == "confirm"`); this is the only prompt-class kind reachable on the
  pinned engine, and it already covers `:confirm()`, the ATTENTION dialog, and
  `inputlist()`.
- Choice-vs-free-text is derived from the paired `cmdline_show` prompt text
  (`PromptState::learn_cmdline`) and from nothing in the `msg_show` itself: a
  prompt string opening with `[` or `(` per accelerator segment is a choice
  prompt; anything else is free text.
- `accepts()` on a choice prompt: the accelerator letters (case-insensitive per
  `:help confirm()`), `<CR>` (bracketed default), `<Esc>` (cancel); all three
